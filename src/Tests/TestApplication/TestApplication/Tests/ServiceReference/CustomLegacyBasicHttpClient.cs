...
namespace TestApplication.Tests.ServiceReference
{
        ...
        IBasicHttpService
    {
        ...
        public void BodyMemberAsync(string messageString, MessageHeader customHeader)
        {
            Message messageInstance = Message.CreateMessage(Endpoint.Binding.MessageVersion ?? MessageVersion.Soap11,
                "urn:BasicHttpService/BodyMember", new BodyMemberBodyWriter(messageString));

            if (customHeader != null)
            {
                messageInstance.Headers.Add(customHeader);
            }
